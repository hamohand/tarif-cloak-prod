...
/**
 * Service pour gérer les requêtes par lots (batch) avec l'API Anthropic.
...
public class AnthropicBatchService {
    ...
    public AnthropicBatchService(
        ...
        log.info("AnthropicBatchService initialisé avec le modèle: {}", model);
    }
     ...
     *
     * @param resultsUrl L'URL des résultats fournie par getBatchStatus()
     * @return La liste des résultats ou une liste vide en cas d'erreur
     */
    public List<BatchResult> getBatchResults(String resultsUrl) {
        if (apiKey == null || apiKey.isBlank()) {
            log.error("Clé API Anthropic non configurée");
    ...
    public static class BatchStatus {
        private String id;
        private String processingStatus; // in_progress, ended
